...
const NGOListingsPage = () => {
    ...
    const fetchCbfRecommendations = async () => {
      try {
        const response = await axios.get(
          `${API_URL}/content-based-recommendations`,
          {
            params: { ngo_id: user._id },
          }
        );
        ...
        setCbfItems(Array.isArray(recommendations) ? recommendations : []);
      } catch (error) {
        console.error("Error fetching recommendations:", error);
        setCbfItems([]);
      }
  ...
  const handleSelect = (listing) => {
    if (selectedListings.some((item) => item.name === listing.name)) {
      setSelectedListings(
        selectedListings.filter((item) => item.name !== listing.name)
      );
    } else {
  ...
  return (
    <div
      className={`container mx-auto p-8 ${isDarkMode ? "bg-gray-900/60 text-white" : "bg-white/60 text-gray-900"}`}
    >
      <div className="mb-8">
        <h2
          className={`text-md md:text-xl text-center font-bold border-2 border-gray-800 rounded-lg uppercase mb-4 ${isDarkMode ? "text-white bg-gray-500" : "text-black bg-gray-300"}`}
        >
          Recommendations
        </h2>
        <div className="flex overflow-x-auto">
          {recommendedItems &&
            ...
            recommendedItems.map((listing, idx) => (
              <div
                key={idx}
                className={`card mr-4 md:min-w-[250px] min-w-[150px] ${isDarkMode ? "bg-gray-900/70 text-white" : "bg-white/70 text-gray-900"} rounded-xl p-4 shadow-lg`}
              >
                <img
                  src={imageUrls[idx % 10]}
                  alt={listing.name}
                  className="object-cover w-full h-32 md:h-48 sm:w-auto sm:max-w-full"
                />
                <h2
                  className={`text-md md:text-lg font-semibold mt-4 ${isDarkMode ? "text-gray-200" : "text-gray-600"}`}
                >
                  {listing.name}
                </h2>
                <div
                  className={`mt-4 p-2 h-12 rounded-sm w-full ${isDarkMode ? "bg-gray-600 text-gray-200" : "bg-gray-300 text-gray-600"}`}
                >
                  <div className="grid grid-cols-3 gap-4">
                    <div className="flex flex-col gap-1">
                      <span
                        className={`${isDarkMode ? "text-gray-300" : "text-gray-800"} text-xs font-semibold`}
                      >
                        Quantity
                      </span>
                      <span
                        className={`${isDarkMode ? "text-gray-300" : "text-gray-800"} text-xs font-bold truncate`}
                      >
                ...
                >
                  {selectedListings.some((item) => item.name === listing.name)
                    ? "Unselect"
                    : "Select"}
                </button>
              </div>
        ...
        <button
          onClick={handleRecommendedItemsRequest}
          className={`mt-4 p-2 text-xs md:text-md bg-blue-700 text-white rounded-md ${isDarkMode ? "w-14 h-8" : "w-19 h-10"}`}
        >
          Request
        </button>
      </div>
      <div className="mb-8">
        ...
        <button
          onClick={handleCbfRecommendedItemsRequest}
          className={`mt-4 p-2 text-xs md:text-md bg-blue-700 text-white rounded-md ${isDarkMode ? "w-14 h-8" : "w-19 h-10"}`}
        >
          Request
        </button>
