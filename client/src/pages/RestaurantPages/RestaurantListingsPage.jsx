...
const RestaurantListingsPage = () => {
    ...
    if (selectedListing) {
      try {
        ...
        const response = await axios.put(
          `${API_URL}/listings/${selectedListing._id}`,
          updatedListing
        );
  ...
  return (
    ...
    >
      ...
      <div className="mb-8">
        <h1
          className={`text-2xl font-semibold mb-4 ${isDarkMode ? "text-white" : "text-gray-600"}`}
        >
          Add New Listing
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {listings.map((listing, index) => (
          <div
            key={index}
            className={`card ${isDarkMode ? "bg-gray-900/70 text-white" : "bg-white/70 text-gray-900"} rounded-xl p-4 shadow-lg`}
          >
            ...
            <h2
              className={`text-lg font-semibold mt-4 ${isDarkMode ? "text-gray-300" : "text-gray-800"}`}
            >
              {listing.name}
            </h2>
            <div
              className={`mt-4 p-2 rounded-sm w-full ${isDarkMode ? "text-gray-300 bg-gray-600" : "text-gray-800 bg-gray-100"}`}
            >
              <div className="grid grid-cols-3 gap-4">
                <div className="flex flex-col gap-1">
                  ...
                  <span
                    className={`text-xs font-bold ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}
                  >
                    {listing.quantity}
                  </span>
                </div>
            ...
            {listing.view !== "blocked" && (
              <div className="flex justify-between mt-4">
                <button
                  onClick={() => handleOpenUpdateModal(listing)}
          ...
          <div className="bg-white text-black p-8 rounded-md relative">
            ...
            <div className="grid grid-cols-4 gap-4">
              ...
              <button
                ...
                className={`col-span-4 p-2 bg-blue-500 text-white rounded-md`}
              >
                Update
              </button>
            </div>
