...
const RestaurantTransactionsPage = () => {
  ...
  const handleCancel = async () => {
    try {
      const response = await axios.put(
        `${API_URL}/orders/${selectedOrder._id}/cancel`,
        {
          code: cancelCode,
  ...
  const handleFulfill = async () => {
    try {
      const response = await axios.put(
        `${API_URL}/orders/${selectedOrder._id}/fulfill`,
        {
          code: fulfillCode,
          user_type: user.userType,
        }
  ...
  const postReview = async () => {
    try {
      const response = await axios.post(
        `${API_URL}/addRestReview/${selectedOrder._id}`,
        {
          review: reviewText,
        }
      );
  ...
  return (
    <div
      className={`container mx-auto p-8 pb-24 ${isDarkMode ? "bg-gray-900/60 text-white" : "bg-white/60 text-gray-900"}`}
    >
      {loading && (
      ...
      {orders.map((order, index) => {
        ...
        return (
          <div
            key={order._id}
            className={`order-card shadow-lg p-4 mb-4 flex flex-col md:flex-row items-center relative ${isDarkMode ? "bg-gray-900/70 text-white hover:bg-gray-800/80" : "bg-white/70 text-gray-900 hover:bg-gray-100/80"} transition duration-300 ease-in-out rounded-xl`}
          >
            <button
            ...
            <img
              ...
              alt="Order"
              className="h-1/2 w-auto mb-4 md:w-1/4 md:h-auto md:mr-4 rounded-md"
            />
            <div className="h-1/2 w-full md:w-3/4 flex flex-col">
              <p className="text-md uppercase md:text-lg font-bold">
                NGO: {order.ngoName}
              </p>
              {order.status === "requested" && (
              ...
              {order.status === "accepted" && (
                <div className="flex mt-2">
                  ...
                  <div className="w-1/2 md:ml-4">
                    <label
                      className={`block text-xs font-medium md:text-sm text-gray-700 ${isDarkMode ? "text-gray-300" : "text-gray-600"}`}
                    >
                      Unique Restaurant Code:
                    </label>
              ...
              {order.rest_review && (
                <div
                  className={`bg-gray-100 p-2 md:p-4 rounded-md mt-2 ${isDarkMode ? "bg-gray-600 hover:bg-gray-500 transition duration-300 ease-in-out" : "hover:bg-gray-200 transition duration-300 ease-in-out"} relative`}
                >
                  {/* Display sentiment label only if reviewSentiment is not an empty string */}
                  {order.rest_sentiment && (
                    <span
                      className={`absolute top-2 right-2 text-xs font-semibold ${order.rest_sentiment === "Positive" ? "bg-green-500 text-white" : "bg-red-500 text-white"} p-1 rounded-md`}
                    >
                      {order.rest_sentiment}
                    </span>
                  )}

                  <p className="text-xs md:text-sm font-semibold hover:bg-gray-200 transition duration-300 ease-in-out">
                    Restaurant Review:
                  </p>
              ...
              {order.ngo_review && (
                ...
                >
                  ...
                  {order.ngo_sentiment && (
                    <span
                      className={`absolute top-2 right-2 text-xs font-semibold ${order.ngo_sentiment === "Positive" ? "bg-green-500 text-white" : "bg-red-500 text-white"} p-1 rounded-md`}
                    >
                      {order.ngo_sentiment}
                    </span>
                  )}
              ...
              {(order.status === "accepted" ||
                ...
                <Link to={`/chat/${order._id}`} className="block w-full mt-2">
                  <button className="btn btn-blue px-2 py-1 md:px-4 md:py-2 w-full font-bold rounded-md bg-blue-500 hover:bg-blue-600 text-white hover:text-white transition duration-300 ease-in-out">
                    Chat
                  </button>
                </Link>
              )}
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="modal bg-white p-8 rounded-lg z-50 relative">
            <button
              ...
              className="modal-close absolute top-4 right-4 text-gray-600 hover:text-gray-900"
            >
              &#x2715;
            </button>
            <h2 className="text-xl font-semibold mb-4">
              Listings for Order ID: {selectedOrder._id}
            </h2>
            <ul>
              {selectedOrder.listings.map((listing, index) => (
                <li key={index} className="mb-2">
                  ...
                  <span className="ml-2 font-semibold">Food Type:</span>{" "}
                  {listing.food_type},
                  <span className="ml-2 font-semibold">Expiry:</span>{" "}
                  {listing.expiry} hr
                </li>
              ))}
            </ul>
      ...
      {selectedOrder && showCancelModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="modal bg-white p-8 rounded-lg z-50 relative">
            ...
            {cancelMessage && (
              <p className="mt-4 text-red-500">{cancelMessage}</p>
            )}
          </div>
        </div>
      )}
      ...
      {selectedOrder && showFulfillModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="modal bg-white p-8 rounded-lg z-50 relative">
            ...
            {fulfillMessage && (
              <p className="mt-4 text-green-500">{fulfillMessage}</p>
            )}
          </div>
        </div>
      )}
      ...
      {selectedOrder && showReviewModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="modal bg-white p-8 rounded-lg z-50 relative">
            ...
            <h2 className="text-xl font-semibold mb-4">
              Review for Order ID: {selectedOrder._id}
            </h2>
            <textarea
              className="border p-2 rounded-md w-full mb-4"
            ...
            {reviewMessage && (
              <p className="mt-4 text-green-500">{reviewMessage}</p>
            )}
          </div>
        </div>
      )}
