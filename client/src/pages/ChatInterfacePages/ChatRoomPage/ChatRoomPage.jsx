...
const ChatRoomPage = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      // API responds with { status, data }; unwrap the order payload
  ...
  return (
    <div
      className={`container mx-auto p-8 flex flex-col h-screen ${isDarkMode ? "bg-gray-900/60 text-white" : "bg-white/60 text-gray-900"}`}
    >
      {orderDetails && (
        <div
          className={`flex flex-col order-details p-4 rounded-lg shadow-md mb-4 ${isDarkMode ? "bg-gray-900/70 text-white hover:bg-gray-800/80" : "bg-white/70 text-gray-900 hover:bg-gray-100/80"} transition duration-300 ease-in-out`}
        >
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold">Order #{orderId}</h2>
            <div
              className={`text-xs md:text-lg font-semibold py-1 px-2 text-white rounded capitalize ${
          ...
          <p className="font-semibold">
            Restaurant: {orderDetails.restaurantId?.username || "Unknown"}
          </p>
          <p className="font-semibold">
            NGO: {orderDetails.ngoId?.username || "Unknown"}
          </p>
          {user?.userType === "restaurant" &&
            orderDetails.status === "requested" && (
              <div className="flex mt-2">
                <button onClick={handleAccept} className="btn btn-green mr-2">
                  Accept
                </button>
                <button onClick={handleDecline} className="btn btn-red">
                  Decline
                </button>
              </div>
            )}
                  ...
                  <input
                    ...
                    value={
                      currentUserType === "Restaurant"
                        ? orderDetails.restCode
                        : orderDetails.ngoCode
                    }
                    readOnly
                  />
                  <button
      ...
      {orderDetails && showModal && (
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div
            className={`modal ${isDarkMode ? "bg-gray-900/90 text-white" : "bg-white/90 text-gray-900"} p-8 rounded-lg z-50 relative`}
          >
            <button
              onClick={() => setShowModal(false)}
              className={`modal-close absolute top-4 right-4 ${isDarkMode ? "text-gray-300 hover:text-white" : "text-gray-600 hover:text-gray-900"}`}
            >
              &#x2715;
            </button>
            <h2 className="text-xl font-semibold mb-4">
              Listings for Order ID: {orderDetails._id}
            </h2>
            <ul>
              {(Array.isArray(orderDetails.listings)
                ? orderDetails.listings
                : []
              ).map((listing, index) => (
      ...
      <div className="flex-grow overflow-y-auto px-4 py-2">
        {(Array.isArray(chatMessages) ? chatMessages : []).map(
          (message, index) => (
            <div
              key={index}
            ...
            >
              {message.sender !== user?._id && (
                <span className="message-sender mr-2 font-bold">
                  {message.sender}:{" "}
                </span>
              )}
      ...
      {isOrderAccepted && (
        <div
          className={`chat-input-form flex py-2 px-3 border rounded-lg shadow-lg border-gray-200 ${isDarkMode ? "bg-gray-900/70 text-white" : "bg-white/70 text-gray-900"}`}
        >
          <input
            type="text"
