...
def get_order_info(order_id):
    try:
        ...
        # Base serialized order document
        order_dict = serialize_doc(order.to_mongo().to_dict())

        # Attach lightweight user info for UI display
        restaurant_user = order.restaurant_id
        ngo_user = order.ngo_id
        order_dict["restaurantId"] = {
            "_id": str(restaurant_user.id) if restaurant_user else None,
            "username": getattr(restaurant_user, "username", None),
        }
        order_dict["ngoId"] = {
            "_id": str(ngo_user.id) if ngo_user else None,
            "username": getattr(ngo_user, "username", None),
        }
...
def add_rest_review(order_id):
    try:
        ...
        # Fetch the order by order_id restaurant already exists
